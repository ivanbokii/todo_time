    ...
    return Token = (function() {
      ...
      Token.prototype._readTokenFrom = function(inputReader) {
        ...
        if (this.token === 'undef' || this.token === 'colon') {
          ...
          this.value = this.raw;
          this.position = inputReader.position;
          this.length = 1;
        ...
        this.length = inputReader.position - this.position;
...
(function() {
  ...
  define('fuzzy_patterns_analyser',['fuzzy_patterns/simple_time', 'fuzzy_patterns/simple_time_modifier', 'fuzzy_patterns/number_time_modifier', 'duration_patterns/simple_unit'], function(SimpleTime, SimpleTimeModifier, NumberTimeModifier, DurationSimpleUnitPattern) {
    ...
    return FuzzyPatternsAnalyser = (function() {
      ...
      FuzzyPatternsAnalyser.prototype.analyse = function(tokens, input) {
        var analysedTokens, complexDurationResult, durationResult, inputResult, modifiedInput, simpleDurationResult, timePattern, timeResult;
        analysedTokens = this._markTimePatternsIn(tokens);
        complexDurationResult = this._analyseComplexDuration(analysedTokens);
        timeResult = null;
        durationResult = null;
        modifiedInput = input;
        if (complexDurationResult) {
          ...
          modifiedInput = this._removeParsedFromInput(input, complexDurationResult.position, complexDurationResult.length);
        } else {
          timePattern = _.find(analysedTokens, function(t) {
            return t.token === 'time_pattern';
          });
          if (timePattern !== void 0) {
            timeResult = timePattern.value.transform(timePattern.raw);
            simpleDurationResult = this._analyseSimpleDuration(tokens);
            modifiedInput = this._removeParsedFromInput(input, timePattern.position, timePattern.length);
            if (simpleDurationResult) {
              durationResult = simpleDurationResult.duration;
              modifiedInput = this._removeParsedFromInput(modifiedInput, simpleDurationResult.position - timePattern.length, simpleDurationResult.length);
            }
          }
        }
        inputResult = this._clean(modifiedInput);
        return {
          time: timeResult,
          duration: durationResult,
          modifiedInput: inputResult
        };
      };

      FuzzyPatternsAnalyser.prototype._analyseComplexDuration = function(tokens) {
        var duration, durationSequence, firstHoursAndMinutes, firstTime, hours, hoursValid, isFound, minutes, minutesValid, secondHoursAndMinutes, secondTime, sequence, tokensToAnalyse;
        ...
        while (tokens.length >= 4) {
          ...
          if (sequence === durationSequence && tokensToAnalyse[0].raw === 'from' && tokensToAnalyse[2].raw === 'to') {
            ...
            if (!(hoursValid || minutesValid)) {
              ...
              return {
                ...
                length: this._getTokensLength(tokensToAnalyse)
              };
            }
          }
      ...
      FuzzyPatternsAnalyser.prototype._analyseSimpleDuration = function(tokens) {
        var durationSequence, isFound, pattern, sequence, tokensToAnalyse;
        sequence = _.pluck(tokens, 'token').join(' ');
        ...
        while (tokens.length >= 3) {
          ...
          sequence = _.pluck(tokensToAnalyse, 'token').join(' ');
          if (sequence === durationSequence && tokensToAnalyse[0].raw === 'for') {
      ...
      FuzzyPatternsAnalyser.prototype._markTimePatternsIn = function(tokens) {
        var biggestLength, newToken, pattern, processedTokens, sequence, smallestLength, tokensToAnalyse, _i, _len, _ref;
        processedTokens = [];
        smallestLength = _.chain(this.timePatterns).min(function(tp) {
          return tp.length;
        }).value().length;
        biggestLength = _.chain(this.timePatterns).max(function(tp) {
          return tp.length;
        }).value().length;
        while (tokens.length >= smallestLength) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            ...
            sequence = _.map(tokensToAnalyse, function(ta) {
              return ta.token;
            }).join(' ');
            if (pattern.check(sequence) && pattern.validate(tokensToAnalyse)) {
      ...
      FuzzyPatternsAnalyser.prototype._getTokensLength = function(tokens) {
        var lastTokenLength, lastTokenPosition;
        lastTokenPosition = tokens[tokens.length - 1].position;
        lastTokenLength = tokens[tokens.length - 1].length;
        return lastTokenPosition + lastTokenLength - tokens[0].position;
      };

      FuzzyPatternsAnalyser.prototype._removeParsedFromInput = function(input, position, length) {
        var substrToRemove;
        substrToRemove = input.slice(position, position + length);
        return input.replace(substrToRemove, '');
      };

      FuzzyPatternsAnalyser.prototype._clean = function(input) {
        input = input.replace('at  ', '');
        if (input.substring(input.length - 3) === 'at ') {
